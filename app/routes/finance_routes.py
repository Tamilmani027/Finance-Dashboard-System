...
def create(payload:FinanceCreate,db=Depends(get_db),current_user=Depends(get_current_user)):
	return finance_service.create_record(db,current_user.id,payload)

@router.get("/",response_model=list[FinanceResponse])
def get_all(db=Depends(get_db),current_user=Depends(get_current_user)):
	return finance_service.get_all_records(db,current_user.id)

@router.get("/{record_id}",response_model=FinanceResponse)
def get_single_record(record_id:int,db=Depends(get_db),current_user=Depends(get_current_user)):
	return finance_service.get_record_by_id(db,record_id,current_user.id)

@router.put("/{record_id}")
def update_record(record_id:int,payload:FinanceUpdate,db=Depends(get_db),current_user=Depends(get_current_user)):
	return finance_service.update_record(db,record_id,current_user.id,payload)

@router.delete("/{record_id}",response_model=dict)
def delete_record(record_id:int,db=Depends(get_db),current_user=Depends(get_current_user)):
